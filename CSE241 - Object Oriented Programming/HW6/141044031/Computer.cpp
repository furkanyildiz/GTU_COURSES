...
Computer::Computer() {
    setOption(0);
    CPU pc_CPU;
    CPUProgramDyn pc_CPUProgram;
    Memory pc_Memory;
    setCPU(pc_CPU);
    setCPUProgramDyn(pc_CPUProgram);
    setMemory(pc_Memory);
...
Computer::Computer(int option) :Computer() {
    ...
    pcCPUProgramDyn.setOption(option);
...
*/
Computer::Computer(CPU myCPU, CPUProgramDyn myCPUProgram, Memory myMemory, 
                                                                int option)
{
    isOptionValid(option);
    setCPU(myCPU);
    setCPUProgramDyn(myCPUProgram);
    setMemory(myMemory);
    setOption(option);
    pcCPU.setOption(option);
    pcCPUProgramDyn.setOption(option);
...
void Computer::setCPUProgramDyn(CPUProgramDyn myCPUProgram){ 
    pcCPUProgramDyn =  myCPUProgram;
}
...
void Computer:: execute(){
    ...
    if(pcCPUProgramDyn.size()<1){
        cerr<<"Program can not run, you have to read file first!"<<endl;
        exit(1);
    }
    while(!pcCPU.halted()){
        string instruction = pcCPUProgramDyn.getLine(pcCPU.getPC() -1 );
...
void Computer:: execute(const string& filename){
    pcCPUProgramDyn.ReadFile(filename);
    while(!pcCPU.halted()){
        string instruction = pcCPUProgramDyn.getLine(pcCPU.getPC() -1 );
        if(getOption() == 1 || getOption() == 2)
            cout << instruction<<endl;
